...
class UniTehran:
    ...
    def get_journals(self):
        ...
        while True:
            ...
            params['page'] = page
            response = get(url=url, params=params, headers=self.headers, data=self.payloads)
    ...
    def complete_profile(self, profile_url: str):
        ...
        def _email(source: BeautifulSoup):
            email_sibling = source.find('span', {'class': 'term'}, text=' پست الکترونیکی ')
            if email_sibling:
        ...
        def _resume(source: BeautifulSoup):
            ...
            if resume_sibling:
                resume = resume_sibling.find_next_sibling('h6', {'class': 'description'})
                if resume:
        ...
        def _scopus(source: BeautifulSoup):
            ...
            if scopus_icon:
                scopus_statistics = scopus_icon.find_next_sibling('ul', {'class': 'content-statistics'})
                citations = scopus_statistics.find('span', {'class': 'label'}, text=' ارجاعات ').find_previous_sibling('span', {'class': 'count increment-counter'}).text if scopus_statistics.find('span', {'class': 'label'}, text=' ارجاعات ') else None
                h_index = scopus_statistics.find('span', {'class': 'label'}, text='h-Index').find_previous_sibling('span', {'class': 'count increment-counter'}).text if scopus_statistics.find('span', {'class': 'label'}, text='h-Index') else None
                scopus_date = source.find('div', {'class': 'sc-date'}).find('span', {'class': 'date-value'}).text if source.find('div', {'class': 'sc-date'}) else None
                model['Citations'] = int(citations) if citations else None
                model['HIndex'] = int(h_index) if h_index else None
                model['Until'] = str(helper.persian_to_datetime(scopus_date.strip())) if scopus_date else None
            return model
        ...
        def _scholar(source: BeautifulSoup):
            ...
            if scholar_icon:
                scholar_statistics = scholar_icon.find_next_sibling('ul', {'class': 'content-statistics'})
                citations = scholar_statistics.find('span', {'class': 'label'}, text=' ارجاعات ').find_previous_sibling('span', {'class': 'count increment-counter'}).text
                h_index = scholar_statistics.find('span', {'class': 'label'}, text='h-Index').find_previous_sibling('span', {'class': 'count increment-counter'}).text
                scholar_date = source.find('div', {'class': 'gsc-date'}).find('span', {'class': 'date-value'}).text
        ...
        def _graduate(source: BeautifulSoup):
            temp = []
            for case in source.find_all('div', {'class': 'rendering rendering_personeducation rendering_compact rendering_personeducation_compact'}):
                degree = case.find('p').find('span').text.split(' ،')[0].strip() if case.find('p').find('span') else None
                university_field = case.find('span', {'class': 'rendering rendering_inline rendering_ueoexternalorganisation rendering_ueoexternalorganisation_inline'}).text.split('،') if case.find('span', {'class': 'rendering rendering_inline rendering_ueoexternalorganisation rendering_ueoexternalorganisation_inline'}) else None
                start_end = case.find_all('p')[1].text.split('←') if len(case.find_all('p')) > 1 else []
                start_end = start_end if len(start_end) == 2 else ['', '']
                temp.append({
                    'Degree': degree,
                    'University': university_field[0].strip() if university_field else None,
                    'Field': university_field[1].strip() if university_field else None,
        ...
        def _keywords(source: BeautifulSoup):
            section_string = source.find('section', {'class': 'page-section content-relation-section person-publications'})
            if section_string:
                script_string = section_string.find('script')
                if script_string:
                    string = script_string.text
                    if string:
                        keywords = string[string.find('[{type:"wordcloud",data:')+24:string.find(',name:"Occurrences"}]')]
                        if keywords:
                            return helper.convert_string_to_json(keywords)

            return None

        source = BeautifulSoup(get(url=profile_url, headers=self.headers, data=self.payloads).text, 'html.parser')
    ...
    def get_profiles(self):
        ...
        for end_point in [SearchType.Profile, SearchType.Retired, SearchType.Decedent]:
            ...
            params['searchType'] = 'dead' if end_point==SearchType.Decedent else end_point.value
            ...
            while True:
                ...
                if not response.json().get('results', None):
                    ...
                    for result in response.json()['results']:
                        ...
                        profile = {
                            'TeacherId': int(result.get('teacherId')) if result.get('teacherId') else None,
                            'Scholarly': [{
                                'Year': int(s.get('year')) if s.get('year') else None,
                                'FirstCount': int(s.get('count1')) if s.get('count1') else None,
                                'SecondCount': int(s.get('count2'))  if s.get('count2') else None
