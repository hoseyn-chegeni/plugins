...
class UniBeheshti:
    ...
    def __activities(self, profile_id: str):
        source = BeautifulSoup(
            get(
                url=f"{self.base_url}/~{profile_id}/activities", data=self.payloads
            ).text,
            "html.parser",
        )
        ...
        for activity in activities:
            ...
            if date:
                if date.text:
                    start = (
                        int(date.text.split("←")[0])
                        if date.text.split("←")[0].isdigit()
                        else date.text.split("←")[0].strip()
                    )
                    end = (
                        int(date.text.split("←")[1])
                        if date.text.split("←")[1].isdigit()
                        else date.text.split("←")[1].strip()
                    )

            yield {
                "Title": title.text.strip() if title else None,
                "SubtitleLabel": (
                    subtitle_label.text.strip() if subtitle_label else None
                ),
                "Start": start,
                "End": end,
            }
    ...
    def __articles(self, profile_id: str):
        source = BeautifulSoup(
            get(url=f"{self.base_url}/~{profile_id}/articles", data=self.payloads).text,
            "html.parser",
        )
        articles = source.find("section", {"class": "main"}).find_all(
        ...
        for article in articles:
            ...
            try:
                yield {
                    "Title": (
                        article.find("span", {"class": "title title-en"}).text.strip()
                        if article.find("span", {"class": "title title-en"})
                        else None
                    ),
                    "CoAuthors": (
                        [
                            author.strip()
                            for author in data[0].split(",")
                            if author.strip()
                        ]
                        if data[0]
                        else []
                    ...
                    "Description": (
                        article.find("em").text.strip() if article.find("em") else None
                    ),
                    "Volume": (
                        data[-3].text.strip().replace("\t", "").replace("\n", "")
                        if data[-3]
                        else None
                    ),
    ...
    def __courses(self, profile_id: str, new_courses: bool = True):
        source = BeautifulSoup(
            get(url=f"{self.base_url}/~{profile_id}/courses", data=self.payloads).text,
            "html.parser",
        )
        if new_courses:
            ...
            if base:
                ...
                if courses:
                    for course in courses:
                        yield {"Name": course.text.strip().replace("\u200c", "")}

    def __thesises(self, profile_id: str, degree: Degrees):
        source = BeautifulSoup(
            get(url=f"{self.base_url}/~{profile_id}/thesis", data=self.payloads).text,
            "html.parser",
        )
        ...
        if degree == Degrees.Doctoral:
            ...
            if thesises_body:
                try:
                    ...
                    thesises = (
                        thesises_body.find_next_sibling("div", {"class": "edu-list"})
                        .find("ol")
                        .find_all("li")
                    )
                if thesises:
                    for thesis in thesises:
                        yield {
                            "Title": thesis.find("span", {"class": "title"})
                            .text.strip()
                            .replace("\u200c", " "),
                            "Subtitle": thesis.find("span", {"class": "subtitle"})
                            .text.strip()
                            .replace("\u200c", " "),
    ...
    def __books(self, profile_id: str):
        source = BeautifulSoup(
            get(url=f"{self.base_url}/~{profile_id}/books", data=self.payloads).text,
            "html.parser",
        )
        ...
        if books:
            for book in books:
                ...
                yield {
                    ...
                    .strip(),
                    "Year": (
                        int(book.find("span", {"class": "years"}).text.replace("،", ""))
                        if book.find("span", {"class": "years"})
                        else None
                    ),
                    "University": data[3]
                    .text.split("-")[0]
    ...
    def __researchs(self, profile_id: str):
        source = BeautifulSoup(
            get(url=f"{self.base_url}/~{profile_id}/research", data=self.payloads).text,
            "html.parser",
        )
        researchs_body = source.find("h2", text=" ارتباط با صنعت")
        researchs = (
            researchs_body.find_next_sibling("ol").find_all("li")
            if researchs_body
            else None
        )
        if researchs:
            for research in researchs:
                yield {
                    "Title": research.find("span", {"class": "title"})
                    .text.replace("\u200c", " ")
                    .strip(),
                    "Organization": research.find("span", {"class": "subtitle"})
                    .text.replace("\u200c", " ")
                    .strip(),
                    "Year": (
                        int(
                            research.find("span", {"class": "years"}).text.replace(
                                "،", ""
                            )
                        )
                        if research.find("span", {"class": "years"})
                        else None
                    ),
                }
    ...
    def __prizes(self, profile_id: str):
        source = BeautifulSoup(
            get(url=f"{self.base_url}/~{profile_id}/award", data=self.payloads).text,
            "html.parser",
        )
        prizes_body = source.find("h2", text=" جوایز و افتخارات")
        prizes = (
            prizes_body.find_next_sibling("ol").find_all("li") if prizes_body else None
        )
        if prizes:
            for prize in prizes:
                yield {
                    "Title": prize.find("span", {"class": "title"})
                    .text.replace("\u200c", " ")
                    .strip(),
                    "Year": (
                        int(
                            prize.find("span", {"class": "years"}).text.replace("،", "")
                        )
                        if prize.find("span", {"class": "years"})
                        else None
                    ),
                }
    ...
    def _complete_profile(self, profile_id: str):
        ...
        def _field_major(source: BeautifulSoup):
            ...
            major = (
                field_major.split("/")[0][len(field) + 2 :]
                .strip()
                .replace("\u200c", " ")
            )
            return field.split("/")[0].strip(), (
                major.strip() if major else field.split("/")[1].strip()
            )
        ...
        def _education(source: BeautifulSoup):
            ...
            for education in educations:
                try:
                    ...
                    dates = (
                        education.find("span", {"class": "years"})
                        .text.strip()
                        .split("←")
                        if education.find("span", {"class": "years"})
                        else None
                    )
                    try:
                        yield {
                            ...
                            "City": data[3],
                            "Degree": education.find("span", {"class": "code"})
                            .text.replace(":", "")
                            .strip(),
                            "Field": (
                                f"{data[-3]} {data[-2]}"
                                if len(data) == 7
                                else f"{data[-2]} {data[-1]}"
                            ),
        ...
        source = BeautifulSoup(
            get(url=f"{self.base_url}/~{profile_id}", data=self.payloads).text,
            "html.parser",
        )
        ...
        return {
            ...
            "Thesises": {
                "PHD": [
                    thesis for thesis in self.__thesises(profile_id, Degrees.Doctoral)
                ],
                "Master": [
                    thesis for thesis in self.__thesises(profile_id, Degrees.Master)
                ],
                "Bachelor": [
                    thesis for thesis in self.__thesises(profile_id, Degrees.Bachelor)
                ],
            },
    ...
    def get_profiles(self):
        ...
        while True:
            ...
            params[
                "_ir_sain_university_people_UniversityFacultyListPortlet_currentNumberPage"
            ] = page
            source = BeautifulSoup(
                get(
                    url=f"{self.base_url}/cv",
                    params=params,
