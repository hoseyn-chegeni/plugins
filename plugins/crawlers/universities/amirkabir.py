...
from schemas import (Activity, Book, Course, EducationalRecord, Employee, Honor, Interest,
                     Invention, Membership, Language, Professor, Skill, Thesis, Workshop)
...
class AmirkabirCrawler(University):
    ...
    def get_employees(self):
        ...
        for link in soup.find_all('a',attrs={'style':'font-family:yekanYW; font-size:15px !important;'}):
            yield self.url + link['href']
    
    def get_professors(self):
        response = check_connection(requests.get, self.url + "cv.php?slc_lang=fa&sid=1&show_all=1")
        soup = BeautifulSoup(response.text, 'html.parser')
        for row in soup.find("div", {'class':'table'}).find_all('div',{'class':'row'})[1:]:
    ...
    def get_professor_page(self, link:str):
        ...
        professor = Professor(
            ...
            group = cv_level[2],
            image = self.url + soup.find('img', {'class': 'yekta_thumb cv_photo'})['src'][2:],
            rank = cv_level[0],
            university_link = link
        )

        for item in soup.find('div',{'class':'list_title'}).find_all('div', {'class': 'items'}):
        ...
        if social_soup:
            try:
                professor.socials.orcid(social_soup.find('a',{'class':'orcid masterTooltip '})['href'])
            except: pass
            try:
                professor.socials.scopus(social_soup.find('a',{'class':'scopus masterTooltip '})['href'])
            except: pass
            try:
                professor.socials.twitter(social_soup.find('a',{'class':'twitter masterTooltip '})['href'])
            except: pass
            try:
                professor.socials.google(social_soup.find('a',{'class':'google masterTooltip '})['href'])
            except: pass
            try:
                professor.socials.telegram(social_soup.find('a',{'class':'telegram masterTooltip '})['href'])
            except: pass
            try:
                professor.socials.researchgate(social_soup.find('a',{'class':'researchgate masterTooltip '})['href'])
            except: pass
            try:
                professor.socials.scholar(social_soup.find('a',{'class':'scholar masterTooltip '})['href'])
            except: pass
        ...
        if doctoral_soup:
            for row in doctoral_soup.find_all('div', {'class':'row'})[1:]:
                details = [cell.text.strip() for cell in row.find_all('div',{'class':'cell'})]
                professor.doctoral_thesis.append(Thesis(
        ...
        if masters_soup:
            for row in masters_soup.find_all('div', {'class':'row'})[1:]:
                details = [cell.text.strip() for cell in row.find_all('div',{'class':'cell'})]
                professor.masters_thesis.append(Thesis(
                    authors = [author for author in details[2].split(" و ")],
                    defense_date = details[3],
                    title = details[1]
                ))
        ...
        if invention_soup:
            for row in invention_soup.find_all('div', {'class':'row'})[2:]:
                details = [cell.text.strip() for cell in row.find_all('div',{'class':'cell'})]
                professor.inventions.append(Invention(
                    country = details[3],
        ...
        if language_soup:
            for row in language_soup.find_all('div', {'class':'row'})[1:]:
                details = [cell.text.strip() for cell in row.find_all('div',{'class':'cell'})]
                professor.languages.append(Language(
                    language = details[1],
                    speaking = details[3],
                    translate = details[2]
                ))
        ...
        if educational_soup:
            for row in educational_soup.find_all('div', {'class':'row'})[1:]:
                ...
                professor.educational_records.append(EducationalRecord(
                    city = details[3],
                    country = details[4],
                    degree = degree,
        ...
        if interests_soup:
            for row in interests_soup.find_all('div', {'class':'row'})[1:]:
                details = [cell.text.strip() for cell in row.find_all('div',{'class':'cell'})]
                professor.interests.append(Interest(
        ...
        if membership_soup:
            for row in membership_soup.find_all('div', {'class':'row'})[1:]:
                details = [cell.text.strip() for cell in row.find_all('div',{'class':'cell'})]
                professor.magazines_membership.append(Membership(
                    title = details[1],
                    description = details[2]
                ))
        ...
        if professional_soup:
            for row in professional_soup.find_all('div', {'class':'row'})[1:]:
                details = [cell.text.strip() for cell in row.find_all('div',{'class':'cell'})]
                professor.professional_membership.append(Membership(
                    title = details[1],
                    description = details[2]
                ))
        ...
        if workshops_soup:
            for row in workshops_soup.find_all('div', {'class':'row'})[1:]:
                details = [cell.text.strip() for cell in row.find_all('div',{'class':'cell'})]
                professor.workshops.append(Workshop(
                    title = details[1],
    ...
    def get_employee_page(self):
        response = check_connection(requests.get, self.url + "persons.php?sid=1&slc_lang=fa#person_part")
        soup = BeautifulSoup(response.text, 'html.parser')
        
        soup = soup.find('div', {'id':'default_data'}).find('div', {'style':'clear:both;'})
        department = None
        while True:
            ...
            if not soup:
                break
            if soup.name == 'div':
                a = soup.find('a', {'style':'font-family:yekanYW; font-size:15px !important;'})
                if a:
                    department = a.text.strip()
            elif soup.name == 'a':
                details_soup = soup.find('div',{'class':'yw_prsn_box yw_gradient_black1 yw_linh_normal yw_radius yw_hover yw_black_txt yw_width_350 yw_margin_10 pad5 f12 yw_table_col_1'})
