        ...
        with sync_playwright() as p:
            ...
            while True:
                ...
                for row in rows:
                    ...
                    last_name = row.find("td", {"data-title": "نام خانوادگی"}).get_text(
                        strip=True
                    )
                    first_name = row.find("td", {"data-title": "نام"}).get_text(
                        strip=True
                    )
                    faculty_group = row.find(
                        "td", {"data-title": "دانشکده/گروه"}
                    ).get_text(strip=True)
                    rank = row.find("td", {"data-title": "رتبه"}).get_text(strip=True)

                    # CV download link
                    cv_link_tag = row.find(
